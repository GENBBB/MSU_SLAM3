...
void System::SaveAllTrajectoryEuRoC(const string &filename) {
    cout << endl << "Saving trajectory to " << filename << " ..." << endl;
    /*if(mSensor==MONOCULAR)
    {
        cerr << "ERROR: SaveTrajectoryEuRoC cannot be used for monocular." << endl;
    ...
    int num_fragment = 0;
    for (Map* pMap : vpMaps) {
        string fragment_name = filename + std::to_string(num_fragment);
        num_fragment++;
        SaveTrajectoryEuRoC(fragment_name, pMap);
    }
}
...
void System::SaveAllKeyFrameTrajectoryEuRoC(const string &filename) {
    cout << endl << "Saving keyframe trajectory to " << filename << " ..." << endl;
    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    int num_fragment = 0;
    for(Map* pMap :vpMaps)
    {
        string fragment_name = filename + std::to_string(num_fragment);
        num_fragment++;
        System::SaveKeyFrameTrajectoryEuRoC(fragment_name, pMap);
    }
}
